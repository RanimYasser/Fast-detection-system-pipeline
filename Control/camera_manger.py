...
class SoftTriggerGrabber:
    ...
    # ---------- Branch on camera ID ----------
        if camera_id == "DEV_1AB22C00C5A5":   # BARCODE cam (KEEPING YOUR PREV SETUP)
            ...
            # Trigger config (as in your previous setup)
            try: f('TriggerSelector').set('FrameStart')
            except: pass
            try: f('TriggerMode').set('Off')
            except: pass
            try: f('TriggerSource').set('Software')
            except: pass
            try:
                ta = f('TriggerActivation')
                if hasattr(ta, 'is_writeable') and ta.is_writeable():
                    ta.set('RisingEdge')
            except: pass
            try: f('TriggerMode').set('On')
            except: pass

        elif camera_id == "DEV_1AB22C00C5A7":  # TOP / expiry cam
            # Faster shutter + ROI tuned for date area; leave barcode cam untouched
            try: f('ExposureAuto').set('Off')
            except: pass
            try: f('GainAuto').set('Off')
            except: pass
            try: f('ExposureTime').set(800.0)   # µs ≈ 1/1667 s (tune 400–800)
            except: pass
            try: f('Gain').set(6.0)
            except: pass
            try: f('PixelFormat').set(pixel_format)
            except: pass
            try: f('StreamBufferHandlingMode').set('NewestOnly')
            except: pass
            try: f('AcquisitionFrameRateEnable').set(False)
            except: pass
            try: f('ExposureMode').set('Timed')
            except: pass
            try: f('AcquisitionMode').set('Continuous')
            except: pass
            try: f('Width').set(1280); f('Height').set(810)
            except: pass
            try: f('OffsetX').set(190); f('OffsetY').set(470)
            except: pass

            # Trigger config for top cam
            try: f('TriggerSelector').set('FrameStart')
            except: pass
            try: f('TriggerMode').set('Off')
            except: pass
            ...
            print(f"[Init] Camera {camera_id} not recognized — using defaults")

        # ---------- Start streaming ONCE for both branches ----------
